...
struct DistortionExerciseView: View {
    @StateObject private var vm: DistortionExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: DistortionExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("What type of distortion was applied?")
                .font(.title3.bold())
            ...
            HStack(spacing: 16) {
                ...
                playButton(title: "Processed", systemImage: "bolt.horizontal", action: vm.playProcessed)
            }
            ...
            if vm.isRevealed {
                DistortionVisualizerView(type: vm.round.target)
    ...
    private var moduleColor: Color { DS.moduleColor("Distortion") }
    ...
    private func choiceButton(_ type: DistortionType) -> some View {
