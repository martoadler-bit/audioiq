...
struct EQExerciseView: View {
    @StateObject private var vm: EQExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: EQExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("Which frequency band was changed?")
                .font(.title3.bold())
            ...
            HStack(spacing: 16) {
                playButton(title: "Original", systemImage: "waveform", action: vm.toggleReference)
            ...
            if vm.isRevealed {
                EQVisualizerView(frequency: vm.round.targetFrequency, gain: vm.round.gain)
                    .transition(.scale(scale: 0.9).combined(with: .opacity))
                    .id(vm.round.targetFrequency)
    ...
    private var moduleColor: Color { DS.moduleColor("EQ") }
    ...
    private func choiceButton(_ choice: EQChoice) -> some View {
        let isCorrect = choice.frequency == vm.round.targetFrequency
        let isSelected = choice == vm.selectedChoice
