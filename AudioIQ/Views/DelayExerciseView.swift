...
struct DelayExerciseView: View {
    @StateObject private var vm: DelayExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: DelayExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            HStack(spacing: 16) {
                ...
                playButton(title: "Processed", systemImage: "arrow.triangle.2.circlepath", action: vm.playProcessed)
            }
            ...
            if vm.isRevealed {
                DelayVisualizerView(amount: vm.round.target)
    ...
    private var moduleColor: Color { DS.moduleColor("Delay") }
    ...
    private func choiceButton(_ amount: DelayAmount) -> some View {
