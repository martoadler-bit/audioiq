...
struct PhaseExerciseView: View {
    @StateObject private var vm: PhaseExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: PhaseExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("What's happening to the stereo image?")
                .font(.title3.bold())
            ...
            HStack(spacing: 16) {
                ...
                playButton(title: "Processed", systemImage: "dot.radiowaves.left.and.right", action: vm.playProcessed)
            }
            ...
            if vm.isRevealed {
                PhaseVisualizerView(type: vm.round.target)
    ...
    private var moduleColor: Color { DS.moduleColor("Phase") }
    ...
    private func choiceButton(_ type: PhaseType) -> some View {
