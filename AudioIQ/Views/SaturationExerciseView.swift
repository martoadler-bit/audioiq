...
struct SaturationExerciseView: View {
    @StateObject private var vm: SaturationExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: SaturationExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("How much warmth was added?")
                .font(.title3.bold())
            ...
            HStack(spacing: 16) {
                ...
                playButton(title: "Processed", systemImage: "flame", action: vm.playProcessed)
            }
            ...
            if vm.isRevealed {
                SaturationVisualizerView(type: vm.round.target)
    ...
    private var moduleColor: Color { DS.moduleColor("Saturation") }
    ...
    private func choiceButton(_ type: SaturationType) -> some View {
