...
struct CodecExerciseView: View {
    @StateObject private var vm: CodecExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: CodecExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("What bitrate does this sound like?")
                .font(.title3.bold())
            ...
            HStack(spacing: 16) {
                ...
                playButton(title: "Processed", systemImage: "antenna.radiowaves.left.and.right", action: vm.playProcessed)
            }
            ...
            if vm.isRevealed {
                CodecVisualizerView(type: vm.round.target)
    ...
    private var moduleColor: Color { DS.moduleColor("Codec") }
    ...
    private func choiceButton(_ type: CodecType) -> some View {
