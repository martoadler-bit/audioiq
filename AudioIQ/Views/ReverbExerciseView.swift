...
struct ReverbExerciseView: View {
    @StateObject private var vm: ReverbExerciseViewModel

    init(engine: ProcessingAudioEngine) {
        _vm = StateObject(wrappedValue: ReverbExerciseViewModel(engine: engine))
    }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("What kind of space was applied?")
                .font(.title3.bold())
            ...
            HStack(spacing: 16) {
                ...
                playButton(title: "Processed", systemImage: "building.columns", action: vm.playProcessed)
            }
            ...
            if vm.isRevealed {
                ReverbVisualizerView(type: vm.round.target)
    ...
    private var moduleColor: Color { DS.moduleColor("Reverb") }
    ...
    private func choiceButton(_ type: ReverbType) -> some View {
